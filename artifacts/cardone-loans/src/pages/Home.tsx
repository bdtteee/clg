...
export default function Home() {
  ...
  const faqs = [
    { q: "Who can apply for funding?", a: "Any African individual aged 18+ or registered African business can apply. You must have a valid National ID or passport and be located in any of the 54 African nations we serve." },
    { q: "How long does the approval process take?", a: "Standard approval takes 2–5 business days after documentation and the processing fee are received. Our U.S.- and Canada-based underwriting partners work diligently to provide timely decisions." },
    ...
    { q: "What does the 65% pre-approval mean?", a: "For loan products, our automated eligibility check guarantees 65% of your requested amount pending final verification. For example, if you apply for $20,000, you receive an instant pre-approval for $13,000." },
  ...
  return (
    <div className="flex flex-col min-h-screen overflow-x-hidden">
      ...
      {/* HERO */}
      <section className="relative min-h-[92vh] flex items-center pt-20 pb-16 overflow-hidden bg-[#0B1F3A]">
        <div className="absolute inset-0">
          <img src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&w=1920&q=80" alt="" className="absolute inset-0 w-full h-full object-cover object-center" />
          <div className="absolute inset-0 bg-[#0B1F3A]/88" />
        </div>
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#D4AF37]/20 border border-[#D4AF37]/30 text-[#D4AF37] mb-6">
                <Globe className="h-4 w-4" />
                <span className="text-sm font-semibold tracking-wide">Africa's Gateway to U.S. & Canadian Capital</span>
              </motion.div>
              <motion.h1 initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.5, delay: 0.1 }} className="text-5xl md:text-6xl xl:text-7xl font-display font-extrabold text-white leading-[1.05] mb-6">
                American funding.{" "}<span className="text-[#D4AF37]">African dreams.</span>
              </motion.h1>
              ...
              <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.5, delay: 0.4 }} className="mt-8 flex flex-wrap items-center gap-5 text-sm text-white/60">
                {["No collateral required", "2–5 business day decisions", "Open to all 54 African nations"].map((t, i) => (
                  <div key={i} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-secondary shrink-0" /><span>{t}</span></div>
                ))}
              </motion.div>
            </div>
            <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.3 }} className="hidden lg:block">
              <div className="grid grid-cols-2 gap-4">
                {[
                  { icon: DollarSign, label: "Capital Deployed (USD)", value: 2800, suffix: "M+", prefix: "$", color: "text-[#D4AF37]" },
                  { icon: Users, label: "Clients Served", value: 47000, suffix: "+", prefix: "", color: "text-[#1FA67A]" },
                  { icon: Award, label: "Approval Rate", value: 87, suffix: "%", prefix: "", color: "text-[#D4AF37]" },
                  { icon: Building, label: "African Nations Covered", value: 54, suffix: "", prefix: "", color: "text-[#1FA67A]" },
                ].map((stat, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-colors">
                    <div className={`w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center mb-3 ${stat.color}`}><stat.icon className="h-5 w-5" /></div>
                    <div className="text-3xl font-display font-bold text-white mb-1"><AnimatedCounter end={stat.value} prefix={stat.prefix} suffix={stat.suffix} /></div>
                    <p className="text-white/50 text-sm">{stat.label}</p>
                  </motion.div>
                ))}
              </div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="mt-4 p-5 rounded-2xl bg-[#D4AF37]/15 border border-[#D4AF37]/30">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#D4AF37]/20 flex items-center justify-center"><Globe className="h-5 w-5 text-[#D4AF37]" /></div>
                  <div>
                    <p className="text-white font-bold">Africa → United States & Canada</p>
                    <p className="text-white/60 text-sm">Bridging African talent with institutional capital since 2019</p>
                  </div>
                </div>
              </motion.div>
            </motion.div>
      ...
      <section id="how-it-works" className="py-28 bg-background">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5 }} className="text-center max-w-2xl mx-auto mb-20">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-4"><Zap className="h-4 w-4" /> Simple 4-Step Process</div>
            <h2 className="text-4xl md:text-5xl font-display font-bold text-foreground mb-4">From application to funded — in days</h2>
            <p className="text-muted-foreground text-lg">No complex paperwork. No bank visits. Apply from anywhere in Africa and receive U.S. dollars.</p>
          </motion.div>
              ...
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }} className="relative z-10 flex flex-col items-center text-center group">
                <div className="relative mb-6">
                  <div className="w-28 h-28 rounded-3xl bg-white border border-border shadow-xl shadow-black/5 flex flex-col items-center justify-center group-hover:border-primary/20 group-hover:shadow-primary/10 transition-all duration-300">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-1 ${item.color}`}><item.icon className="h-5 w-5" /></div>
      ...
      <section id="products" className="py-28 bg-primary relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              { icon: User, iconBg: "bg-secondary/20", iconColor: "text-secondary", title: "Personal Grant", subtitle: "Non-repayable funds for personal goals.", range: "$2,000 – $10,000 USD", fee: "KES 1,300 (~$10)", badge: null, features: ["No repayment required", "National ID only", "2–3 day decision"] },
              { icon: Briefcase, iconBg: "bg-accent/20", iconColor: "text-accent", title: "Business Grant", subtitle: "Growth capital for African businesses.", range: "$5,000 – $30,000 USD", fee: "KES 2,600 (~$20)", badge: null, features: ["No repayment required", "Business reg. + tax PIN", "Any business type"] },
              { icon: User, iconBg: "bg-secondary/20", iconColor: "text-secondary", title: "Personal Loan", subtitle: "Flexible USD for significant personal needs.", range: "$10,000 – $50,000 USD", fee: "KES 2,600 (~$20)", badge: "65% Pre-approved", features: ["Instant 65% pre-qualification", "Flexible repayment schedule", "Personal use"] },
              { icon: Briefcase, iconBg: "bg-accent/20", iconColor: "text-accent", title: "Business Loan", subtitle: "Major capital for African enterprise growth.", range: "$20,000 – $100,000 USD", fee: "KES 6,500 (~$50)", badge: "65% Pre-approved", features: ["Instant 65% pre-qualification", "Business expansion ready", "Most popular product"], highlight: true },
            ].map((product, i) => (
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <Card className={`h-full ${product.highlight ? "bg-white/12 border-accent/60 border-2 relative overflow-hidden" : "bg-white/5 border-white/10 hover:bg-white/8"} text-white backdrop-blur-sm transition-all duration-300 hover:shadow-2xl hover:shadow-black/20`}>
                  {product.highlight && <div className="absolute top-0 right-0 bg-accent text-primary text-xs font-bold px-5 py-1.5 rounded-bl-xl tracking-wide">MOST POPULAR</div>}
                  <CardHeader className="pb-4">
                    <div className={`w-12 h-12 rounded-xl ${product.iconBg} flex items-center justify-center mb-4`}><product.icon className={`h-6 w-6 ${product.iconColor}`} /></div>
                    <div className="flex items-start justify-between gap-3">
                      <div><CardTitle className="text-2xl text-white mb-1">{product.title}</CardTitle><CardDescription className="text-white/55 text-base">{product.subtitle}</CardDescription></div>
                      {product.badge && <span className="inline-flex items-center shrink-0 rounded-lg bg-accent/20 px-2.5 py-1 text-xs font-bold text-accent ring-1 ring-accent/30 whitespace-nowrap">{product.badge}</span>}
                    </div>
                  </CardHeader>
                  ...
                  <CardFooter><Link href="/register" className="w-full"><Button className={`w-full font-bold h-11 ${product.highlight ? "bg-accent text-primary hover:bg-accent/90" : "bg-white text-primary hover:bg-white/90"}`}>Apply Now <ArrowRight className="ml-2 h-4 w-4" /></Button></Link></CardFooter>
                </Card>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-24 bg-background">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5 }} className="text-center max-w-xl mx-auto mb-14">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">Why Africans choose Cardone</h2>
            <p className="text-muted-foreground">Built specifically for the African market. Backed by U.S. and Canadian funding partners.</p>
          </motion.div>
              ...
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <Card className="h-full border-border hover:shadow-lg hover:border-primary/20 transition-all duration-300">
                  <CardContent className="p-8">
                    <div className={`w-12 h-12 rounded-2xl ${item.color} flex items-center justify-center mb-5`}><item.icon className="h-6 w-6" /></div>
      ...
      <section className="py-24 bg-muted/30">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5 }} className="text-center max-w-2xl mx-auto mb-14">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary/10 text-secondary text-sm font-semibold mb-4"><TrendingUp className="h-4 w-4" /> Common Funding Uses</div>
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">What Africans use U.S. funding for</h2>
            <p className="text-muted-foreground text-lg">From education to agribusiness, our funding powers real African goals. There are no restrictions on use — personal or business, your vision leads.</p>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              ...
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.07 }}>
                <Card className="h-full border-border hover:shadow-md hover:border-primary/20 transition-all duration-300 text-center group">
                  <CardContent className="p-5">
                    <div className={`w-12 h-12 rounded-2xl ${use.color} flex items-center justify-center mb-4 mx-auto group-hover:scale-110 transition-transform`}><use.icon className="h-6 w-6" /></div>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="mt-10 p-6 rounded-2xl bg-primary/5 border border-primary/15 text-center max-w-2xl mx-auto">
            <p className="text-sm text-muted-foreground"><strong className="text-foreground">No restrictions on use of funds.</strong> Personal Grants and Business Grants are 100% non-repayable. Loans come with full terms disclosed before disbursement. All funding is disbursed in USD.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 bg-background">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5 }} className="text-center max-w-2xl mx-auto mb-14">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-4"><Star className="h-4 w-4" /> Success Stories</div>
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-3">Africans who got funded</h2>
            <div className="flex justify-center gap-1 mb-2">{[...Array(5)].map((_, i) => <Star key={i} className="h-5 w-5 fill-accent text-accent" />)}</div>
            <p className="text-muted-foreground">Rated 4.9/5 across 47,000+ verified applicants across Africa</p>
          </motion.div>
              ...
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }} className="flex flex-col">
                <Card className="h-full border-border hover:shadow-xl hover:border-primary/20 transition-all duration-300 bg-white flex flex-col">
                  <CardContent className="p-7 flex flex-col flex-1">
                    ...
                    <div className="flex gap-1 mb-4">{[...Array(t.rating)].map((_, j) => <Star key={j} className="h-4 w-4 fill-accent text-accent" />)}</div>
                    <p className="text-foreground/80 text-sm leading-relaxed mb-4 flex-1">"{t.quote}"</p>
                    <div className="mt-auto pt-4 border-t border-border">
                      <div className="flex items-center gap-3 mb-2">
                        <div className="w-11 h-11 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold text-base shrink-0">{t.name.charAt(0)}</div>
                        <div><p className="font-semibold text-sm">{t.name}</p><p className="text-muted-foreground text-xs">{t.role} · {t.location}</p></div>
                      </div>
                      <span className="inline-flex items-center gap-1.5 bg-secondary/10 text-secondary text-xs font-semibold px-3 py-1 rounded-full"><CheckCircle2 className="h-3.5 w-3.5" /> {t.product}</span>
                    </div>
                  </CardContent>
                </Card>
              </motion.div>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { value: "47,000+", label: "Africans Served", icon: Users },
              { value: "$2.8B+", label: "Capital Deployed (USD)", icon: DollarSign },
              { value: "87%", label: "Approval Rate", icon: Award },
              { value: "2–5 days", label: "Average Decision Time", icon: Clock },
            ].map((stat, i) => (
              <div key={i} className="flex items-center gap-4 p-5 rounded-2xl bg-muted/40 border border-border">
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><stat.icon className="h-5 w-5 text-primary" /></div>
                <div><p className="text-xl font-display font-bold text-foreground">{stat.value}</p><p className="text-xs text-muted-foreground">{stat.label}</p></div>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      <section id="faq" className="py-24 bg-background">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5 }} className="text-center mb-14">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-4"><HelpCircle className="h-4 w-4" /> Common Questions</div>
            ...
            {faqs.map((faq, i) => (
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.4, delay: i * 0.05 }}>
                <div className={`border rounded-2xl overflow-hidden bg-white transition-all duration-200 ${openFaq === i ? "border-primary/30 shadow-md shadow-primary/5" : "border-border hover:border-primary/20"}`}>
                  <button className="w-full flex justify-between items-center p-6 text-left" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  ...
                  {openFaq === i && <div className="px-6 pb-6 text-muted-foreground leading-relaxed border-t border-border/50 pt-4">{faq.a}</div>}
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-24 bg-primary relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5 }}>
            ...
            <h2 className="text-4xl md:text-5xl font-display font-bold text-white mb-4">Africa to the U.S. — start your application today</h2>
            <p className="text-white/60 text-xl mb-10 max-w-2xl mx-auto">Join 47,000+ Africans who have already accessed U.S. capital through our platform. Create your free account in minutes. Decision in 2–5 business days.</p>
